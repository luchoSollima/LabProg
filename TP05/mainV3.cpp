...
int main()
{
	...
	cout << "\nEste programa arma un tablero de Nonograma a partir del ingreso de los patrones de casillas negras de cada fila. Los valores validos para los numeros del patron son de 0-15." << endl << endl;
	...
	cout << "Ingrese un numero del patron (ingrese 0 si desea dejar la fila vacia)." << endl;
	...
	do
	{
		...
		// Requests the user to input an integer. 

		do
		{
			cin >> userInput;

			// If the input type is not an integer, the program clears the input and asks the user for it again. Else, the loop doesn't iterate again.

			if (cin.fail()) 
			{
				cin.clear();
				cin.ignore(numeric_limits<streamsize>::max(), '\n');
				cout << "Tipo de input no valiado. Ingrese un numero entero:" << endl;
			}
			else 
			{
				correctInput = false;
		...
		if (totalCellNumber > maxCellsPerRow && userInput != 0)
		{
			...
			cout << "Cantidad de casillas excedida. Ingrese nuevamente el numero:" << endl;
		}
	...
	// All comments made for the first loop apply for the following four loops. The only difference is the row it fills.
	...
	// All comments made for the first row print apply for the remaining four rows. The only difference is the row it prints.
